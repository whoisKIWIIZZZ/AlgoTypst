...
如果$n$是2的幂时，MERGESORT 与 BOTTOMUPSORT 两个算法的执行比较次数相同。

合并两个排好序的子数组，所需要的元素比较次数在$n / 2$和$n - 1$之间，所以最小比较次数是：
...
<选择问题寻找第k小元素>
#block[
分组：将 $n$ 个元素每 5 个一组，分成 $floor.l n/5 floor.r$ 组。
...
快排是需要最坏$O \( n \)$、平均 $O\(log n\)$ 的空间来存储递归中的左右边界的。运行时间最坏情况下为$O \( n^2 \)$，平均复杂度为$O \( n log n \)$。

快速选择平均时间复杂度为 $O(n)$，最坏时间复杂度为 $O(n^2)$。由于每次只递归一边，平均递归深度为 $O(log n)$，最坏递归深度为 $O(n)$，因此递归栈空间平均为 $O(log n)$，最坏为 $O(n)$。

如果使用中位数的中位数作为 pivot，则可以保证最坏时间复杂度为 $O(n)$
...
根据多数元素的定义可知，多数元素必定在数组中出现次数最多。而若将原数组分成2部分，假设多数元素存在，则多数元素必定是在其中一部分出现次数最多的元素之一。
...
#unim[
  ...
  现有两种时间复杂度为 $O\(n\)$、空间复杂度为 $O\(1\)$ 的算法：
  
  - 摩尔投票算法：通过维护一个候选元素和计数器，遍历数组来确定多数元素。把当前元素与候选元素比较，如果相同则计数器加一，否则减一。当计数器为零时，更新候选元素。最后返回候选元素。容易想到，多数元素必定能坚持到最后，而最后一个元素不一定是多数元素。所以需要再进行一遍计数来验证候选元素是否为多数元素。只进行两次 $O\(n\)$ 的遍历，只维护一个候选元素与一个计数器，所以时间复杂度为 $O\(n\)$、空间复杂度为 $O\(1\)$。
  - 蒙特卡洛方法：随机选择一个元素，统计它在数组中出现的次数，如果超过 $n/2$ 则返回该元素，否则继续随机选择。假设固定随机选择次数的上限为常数 $k$，则时间复杂度为 $O\(k n\)=O\(n\)$。由于多数元素出现的概率超过 $1/2$，所以进行 $k$ 次随机选择后仍未找到正确答案的概率小于 $2^(-k)$，即使是取 $k=10$，这个概率也非常小。
]
...
#table(
  ...
  // 表头
  [*芯片 $A$ 的真实状态*], [*芯片 $B$ 的真实状态*], [*测试报告结果 ($A arrow B$)*],
  
  // 数据行
  [Good (好)], [Good (好)], [必定报 Good],
  [Good (好)], [Bad (坏)],  [必定报 Bad],
  [Bad (坏)],  [Good (好)], [任意 (可能报 Good，也可能报 Bad)],
  [Bad (坏)],  [Bad (坏)],  [任意 (坏芯片可以故意串供、合谋欺骗)],
)
...
void sol(int x, int y, int size, int st){
    if(size >= 2){
        sol(x, y, size - 1, st);// 1. 左上角：处理前半部分运动员（1、2号）自己内部的比赛
        sol(x + (1 << (size - 1)), y, size - 1, st + (1 << (size - 1)));// 2. 左下角：处理后半部分运动员（3、4号）自己内部的比赛
        sol(x, y + (1 << (size - 1)), size - 1, st + (1 << (size - 1)));// 3. 右上角：把 左下角 的对阵策略，原封不动抄过来
        sol(x + (1 << (size - 1)), y + (1 << (size - 1)), size - 1, st);// 4. 右下角：把 左上角 的对阵策略，原封不动抄过来
    }
    else{
...
=== 向量卷积
<向量卷积>
给定向量$a = \( a_0 \, a_1 \, dots.h \, a_n \) \, b = \( b_0 \, b_1 \, dots.h \, b_n \)$，则有二者的卷积为$c = \( c_0 \, c_1 \, dots.h \, c_(2 n) \)$，其中
...
的正方形，特殊方格必在其中的一个正方形中。然后再中心摆上一个地毯，地毯缺少的一块朝向特殊方格所在正方形，这样相当于每个小正方形中有一个特殊方格。一直分治下去，直到边长为 2 结束。
...
+ 如果一个数加上上一个有效子段得到的结果比这个数大，那么该数也属于这个有效子段。

+ 如果一个数加上上一个有效子段得到的结果比这个数小，那么这个数单独成为一个新的有效子段。

+ 在执行上述处理的过程中实时更新当前有效子段的所有元素之和并取最大值。
...
而如果用分治算法解决这道题，只需要求出左半部分和右半部分的最大子段和，最后计算一定要包含中间的最大子段和，三者取最大值就是答案。
...
#unim[
  上述分治算法的实现的时间复杂度甚至是 $O(n log n)$ 的，不如朴素实现。现有改进方案：定义结构体 `node`，包含四个成员：
  - `sum`（区间和=左区间和+右区间和）；
  - `lmax`（以区间左端点为起点的最大子段和=左区间以左端点为起点的最大子段和与左区间和+右区间以左端点为起点的最大子段和的较大值）；
  - `rmax`（以区间右端点为终点的最大子段和=右区间以右端点为终点的最大子段和与右区间和+左区间以右端点为终点的最大子段和的较大值）；
  - `max`（区间内的最大子段和=左区间内的最大子段和、右区间内的最大子段和、左区间以右端点为终点的最大子段和+右区间以左端点为起点的最大子段和三者的较大值）。
  
对于两个 `node` 结构体 `a` 和 `b`，定义一个合并函数 `merge(a, b)` 来计算合并后的 `node` 结构体。这样在分治过程中，每次合并时只需要调用一次 `merge` 函数，时间复杂度为 $O(1)$，整个算法的时间复杂度为 $O(n)$。

  至于 `merge` 函数的实现，主要是根据 `a` 和 `b` 的四个成员来计算合并后的 `node` 结构体的四个成员。具体实现如下：
  ...
  线段树的基本原理也是基于分治思想。若是采用线段树维护某区间的最大子段和，则可以按照上述 `node` 结构体的定义来构建线段树，每个节点存储一个 `node` 结构体。对于单点修改操作和查询操作，都可以通过调用 `merge` 函数来维护线段树的节点合并，从而实现 $O(log n)$ 的时间复杂度。
]
...
我们可以考虑$w z + x y = \( w + x \) \( y + z \) - w y - x z$，这样就只需要进行3个$n / 2$的子问题了，复杂度为$Theta \( n^(log_2 3) \) = O \( n^1.59 \)$
