...
== 最长公共子序列
<最长公共子序列>
#block[#strong[题面]：给定两个序列 $X = chevron.l x_1 \, dots.h \, x_m chevron.r$ 和 $Y = chevron.l y_1 \, dots.h \, y_n chevron.r$，求它们的最长公共子序列（LCS）的长度。#emph[子序列]是指从原序列中删除若干元素（可能为0个）后保持原有相对顺序的序列。例如，序列 $(1, 3, 5)$ 是 $(1, 2, 3, 4, 5)$ 的子序列，而 $(1, 5, 3)$ 不是。#emph[公共子序列]是指同时是两个序列子序列的序列。]
