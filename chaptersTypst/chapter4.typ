...
$s[-1]$ 表示序列$s$的最后一个元素，$s[: - 1]$
表示$s$去掉最后一个元素后的子序列
$
  "LCS" (s_1 \, s_2) = cases(delim: "{", 0 \, & upright("if ") s_1 upright(" or ") s_2 upright(" is empty"), "LCS" (s_1[:-1] \, s_2[:-1]) + 1 \, & upright("if ") s_1[-1] = s_2[-1], max ("LCS" (s_1[:-1] \, s_2) \, "LCS" (s_1 \, s_2[:-1])) \, & upright("otherwise"))
$

or,$L[i,j] = L[i-1,j-1]+1 "if" "s1"[i] == "s2"[j] "else" max(L[i,j-1],L[i-1,j])$.
```
while (cin >> s1 >> s2){
    memset(dp, 0, sizeof(dp));
    int n1 = strlen(s1), n2 = strlen(s2);
    for (int i = 1; i lt.eq.slant n1; ++i)
        for (int j = 1; j lt.eq.slant n2; ++j)
            if (s1[i - 1] == s2[j - 1])
                dp[i][j] = dp[i - 1][j - 1] + 1;
            else
...
```
时间复杂度是$O(m n)$.观察到$L[i,j]$只需要考察两行(i-1,i),则只需要存两行L即可,空间复杂度是$O(min(n,m))$.

要同时求出最长公共子序列的长度和序列，首先初始化二维数组$c [ m + 1 ] [ n + 1 ]$（存储长度）和$b [ m + 1 ] [ n + 1 ]$（存储方向），将$c$的第一行和第一列初始化为0。然后遍历字符串$X$和$Y$：当$X [ i - 1 ] = Y [ j - 1 ]$时，令$c [i] [j] = c [ i - 1 ] [ j - 1 ] + 1$并在$b [i] [j]$标记\"↖\"；否则比较$c [ i - 1 ] [j]$和$c [i] [ j - 1 ]$，若$c [ i - 1 ] [j] gt.eq.slant c [i] [ j - 1 ]$则令$c [i] [j] = c [ i - 1 ] [j]$且标记\"↑\"，否则令$c [i] [j] = c [i] [ j - 1 ]$且标记\"←\"。最终$c [ m ] [ n ]$即为LCS长度。回溯时从$b [ m ] [ n ]$开始：若标记为\"↖\"则递归$b [ i - 1 ] [ j - 1 ]$并记录$X [ i - 1 ]$；若为\"↑\"则递归$b [ i - 1 ] [j]$；若为\"←\"则递归$b [i] [ j - 1 ]$。最后将记录的字符反转即得LCS序列。
...
$ F [i] = max_(0 lt.eq.slant j < i \, A [j] < A [i]) { F [j] + 1 } $
...
== 矩阵链乘法
<矩阵链乘法>
给定$n$个矩阵的链$chevron.l A_1 \, A_2 \, dots.h \, A_n chevron.r$，其中矩阵$A_i$的维度为$p_(i - 1) times p_i$，求计算该矩阵链乘积所需的最少标量乘法次数。矩阵乘法满足结合律但不满足交换律，因此*不同的括号化方案会导致不同的计算代价*。



设一个由$n$个矩阵组成的序列的可选的括号化方案为$P ( n )$。则$P ( n )$递归式(卡特兰数)为：

$ P ( n ) = cases(delim: "{", 1 & n = 1, sum_(k = 1)^(n - 1) P ( k ) P ( n - k ) & n gt.eq.slant 2) $
...
定义$A_(i : j)$为$A_i A_(i + 1) dots.h.c A_j$的乘积，其中$i lt.eq.slant j$。如果$A_(i : j)$可以继续分割（即$i < j$），利用括号化分割成$A_(i : k)$和$A_(k + 1 : j)$，其中$i lt.eq.slant k < j$。
...
- 当$i < j$时，利用最优括号化方案：
  $ m [ i \, j ] = m [ i \, k ] + m [ k + 1 \, j ] + p_(i - 1) p_k p_j \, quad upright("其中") i lt.eq.slant k < j $

分段函数形式表示为：
$
  m [ i \, j ] = cases(delim: "{", 0 & i = j, min_(i lt.eq.slant k < j) #scale(x: 100%, y: 100%)[{] m [ i \, k ] + m [ k + 1 \, j ] + p_(i - 1) p_k p_j #scale(x: 100%, y: 100%)[}] & i < j)
$
...
利用Floyd检测负权圈:在算法完成后检查距离矩阵对角线元素，满足：
$ exists i in V : d i s t [i] [i] < 0 arrow.r.double upright("存在包含顶点") i upright("的负权圈") $
== 0/1 背包问题
<背包问题>
给定$n$个物品，每个物品有重量 $w_i$ 和价值 $v_i$，背包容量为 $C$。每个物品只能选择装入或不装入（0/1），求能装入背包的物品总价值最大值。

$
 max sum_(i=1)^n v_i x_i  quad
 s.t. 
  sum_(i=1)^n w_i x_i lt.eq.slant C and
...
$
max sum_(i=2)^n v_i x_i quad s.t. 
  sum_(i=2)^n w_i x_i lt.eq.slant C - w_1 y_1 and
  x_i in {0,1} quad i=2\,dots.h\,n

$

考虑反证，固定 $y_1$ 后，假设 $( y_2 \, dots.h.c \, y_n )$ 不是剩余容量子问题的最优解，则存在可行解 $( z_2 \, z_3 \, dots.h.c \, z_n )$，满足
$
  sum_(i = 2)^n v_i z_i > sum_(i = 2)^n v_i y_i\
  w_1 y_1 + sum_(i = 2)^n w_i z_i lt.eq.slant C\
$
...
背包问题的最优解能够在$Theta ( n C )$的时间内和$Theta ( C )$的空间内得到。下面,j是背包里占据了多少容量,i是下标
$
  V[i,j] = cases(
    0 quad & i=0 or j =0,
    V[i-1,j] & "if" j < w_i,
    max{V[i-1,j], V[i-1,j-w_i] + v_i} & "otherwise" 
  )
$
```
v[i][j]=0;
...
根据动态规划原理，存储长度满足：
$
  S [i] = min_(1 lt.eq k lt.eq min { i \, 256 }) {S [ i - k ] + k times b m ( i - k + 1 \, i ) } + 11
$
...
void compress2(int* p, int n, int* s, int* I, int* b)
{
    ...
    for (int i = 1; i lt.eq.slant n; i++) {
        ...
        I[i];
        for (int k = 1; k lt.eq.slant i && k lt.eq.slant Imax; k++) {
...
== 多段图问题
<多段图问题>
给定一个多段有向图，每条边有权重，求从源点到汇点的最短路径（或最大成本路径）。#emph[多段图]是指顶点可以划分为若干个阶段（层），边只能从前一阶段的顶点指向后一阶段的顶点，不存在反向边或同阶段间的边。这种结构使得可以按照阶段顺序进行动态规划，每阶段只需考虑从上一阶段转移过来的最优解。
...
$
  "COST" ( i \, j ) = cases(
    ...
    chevron.l j \, l chevron.r in E) #scale(x: 100%, y: 100%)[{] c ( j \, l ) + "COST" ( i + 1 \, l ) #scale(x: 100%, y: 100%)[}] & upright("当 ") i < k upright("（递推规则）")
  )
$
...
$
  "BCOST" ( i \, j ) = cases(
    ...
    chevron.l l \, j chevron.r in E) "BCOST" ( i - 1 \, l ) + c ( l \, j )  & upright("递推情况")
  )
$

== 最优二分检索树问题
<最优二分检索树问题>
给定$n$个有序关键字$a_1 < a_2 < dots.h < a_n$，每个关键字$a_i$被检索的概率为$p_i$，未命中区间$E_j$的概率为$q_j$。构造一棵二分检索树，使得加权平均检索次数最小$ min (sum_(i = 1)^n p_i dot.op ("depth"(a_i ) + 1) + sum_(j = 0)^n q_j dot.op ("depth"(E_j ) + 1)) $.

二叉搜索树是一棵二叉树，每个节点存储一个关键字，左子树的关键字都小于节点关键字，右子树的关键字都大于节点关键字。检索时从根节点开始，与节点关键字比较，相等则命中，不等则进入左或右子树继续比较。叶子节点表示区间,非叶子节点表示点;返回值如果搜索到了叶子,返回的是区间;如果不是,就返回的是值.

深度从0开始.例如根节点深度是0.

记
$
  w_(i j) = sum_(s = i)^j p_s + sum_(s = i - 1)^j q_s
$
表示子问题$a_i, dots.h, a_j$中所有成功概率和失败概率之和。记$m[i,j]$表示关键字$a_i, dots.h, a_j$构成的最优二分检索树的最小期望检索代价，最终要求$m[1,n]$。

若选择$a_k$作为根，则左子树为$a_i, dots.h, a_(k - 1)$，右子树为$a_(k + 1), dots.h, a_j$。由于左右子树接到根下面后，所有节点深度都增加$1$，所以会额外增加一次概率总和$w_(i j)$。因此有：
$
  m[i,j] = w_(i j) + min_(i lt.eq.slant k lt.eq.slant j) (m[i,k - 1] + m[k + 1,j])
$

```
ALGORITHM OptimalBST(p, q, n)
    //   p[1..n] - 成功检索关键字 a_i 的概率, q[0..n] - 失败检索虚拟叶子 E_j 的概率
    //   n       - 关键字的数量
    // 输出:
    //   m[1..n+1, 0..n] - 存储子树的最小期望检索代价
    //   root[1..n, 1..n] - 存储子树的根节点位置，用于重构最优树
    for i = 1 to n + 1 do
        m[i, i - 1] = q[i - 1]
        w[i, i - 1] = q[i - 1]
    end for
...
== TSP问题
给定$n$个城市和它们之间的距离矩阵$D$，求一条最短的巡回路径，使得每个城市恰好访问一次并返回起点.

记状态变量$S_k eq.delta (S,i),S subset {2,3, dots,i-1,i+1,dots,n} and abs(S) = k$代表第k阶段到达城市 i 时途中所经过的k个城集合.

则$
f_k (S,i) = min_(j in S) (D_(j,i) + f_(k-1) (S_k\\j,j))\
f_0(phi.alt,i) = D_(1,i)
$

== 流水线调度问题
n个物体,在M1和M2两个机器(有顺序)先后加工,时间分别为$a_i,b_i$.求一个顺序,使得用的总时间最小.

令$N_1 = {i|a_i lt b_i}$,$N_2 = {i|a_i gt.eq.slant b_i}$.
将N_1和N_2分别排序,前者依照$a_i$的非递减序,后者是$b_i$的非递增序.然后得到一个排列,工作顺序是先完成N1然后完成N2(也就是,concat)
...
+ #strong[构造矛盾]：
  ...
  - 存在更优路径$p'_(i j)$满足$w ( p'_(i j) ) < w ( p_(i j) )$,$p' = chevron.l v_1 \, dots.h \, v_(i - 1) chevron.r compose p'_(i j) compose chevron.l v_(j + 1) \, dots.h \, v_k chevron.r$

  - 计算总权重：
    $ w ( p' ) = w ( p ) - w ( p_(i j) ) + w ( p'_(i j) ) < w ( p ) $ 这与$p$是最短路径的初始假设矛盾，故原命题得证。
...
$mono("dp") [i] [ m ]$表示以$i$为根的子树保留$m$个节点的最小代价 $ mono("dp") [i] [ m ] = 0 quad forall i > floor.l n \/ 2 floor.r $


$ mono("dp") [i] [j] = cases(delim: "{", w_(i \, 2 i) + w_(i \, 2 i + 1) + min_a mono("dp") [ 2 i ] [ a ] + min_b mono("dp") [ 2 i + 1 ] [ b ] upright("(只有根)") quad & j = 1, min {w_(i \, 2 i) + min mono("dp") [ 2 i ] [ a ] + mono("dp") [ 2 i + 1 ] [ j - 1 ] upright("（断左）")),
