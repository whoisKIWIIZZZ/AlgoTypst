...
```
时间复杂度是$O(n^2)$.观察到$L[i,j]$只需要考察两行(i-1,i),则只需要存两行L即可,空间复杂度是$O(min(n,m))$.
== 最长上升子序列
<最长上升子序列>
...
$
  sum_(i = 2)^n v_i z_i gt.eq.slant sum_(i = 2)^n v_i y_i\
  w_1 y_1 + sum_(i = 2)^n w_i z_i lt.eq.slant C\
$

$ ==> v_1 y_1 + sum_(i = 2)^n w_i z_i gt.eq.slant sum_(i = 1)^n v_i y_i $
...
$
  "BCOST" ( i \, j ) = cases(
    ...
    angle.l l \, j angle.r in E) "BCOST" ( i - 1 \, l ) + c ( l \, j )  & upright("递推情况")
  )
$

== 最优二分检索树问题
<最优二分检索树问题>
给定$n$个有序关键字$a_1 < a_2 < dots.h < a_n$，每个关键字$a_i$被检索的概率为$p_i$，未命中区间$E_j$的概率为$q_j$。构造一棵二分检索树，使得加权平均检索次数最小$ min (sum_(i = 1)^n p_i dot.op ("depth"(a_i ) + 1) + sum_(j = 0)^n q_j dot.op "depth"(E_j )) $.
...
记$w_(i j) = sum_(s = i-1)^(j-1)(a_s+b_s)+a_j$,平均检索次数为$p_(i j)$,那么我们要求$w_(1 n)p_(1 n)$.

容易得到$w_(i j)p_(i j) = w_(i j) + w_(i  ,m-1)p_(l) + w_(m+1,j)p_r$,l,r为左右子树.
那么级$m[i j] = w_(i j)p_(i j) $,有
$
