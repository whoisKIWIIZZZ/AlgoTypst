...
一方面,
  $
    lambda \[ y \] & lt.eq lambda \[ w \] quad upright("（因为") y upright("先于") w upright("被选中）。因为在当前这一步，算法面临很多未处理的节点（包括 y 和 w），但算法最终优先挑选了 y。根据优先队列“取最小值”的规则，当前 y 的估计值必然小于或等于 w 的估计值。") \
                   & lt.eq lambda \[ x \] + upright("length") \( x \, w \) quad upright("（x 已经被处理过了，松弛操作保证）") \
...
反证.假定选了一个$e in E and  e in.not T_min$,令$T' = T_min union {e}$.令$e = ⟨ x,y ⟩$,那么x,y构成了一个环.删掉环里权值最大的边,得到$T'' (eq.not T_min)$,而且权值更小,矛盾.
...
如果活动$a_i$和$a_j$的时间区间$\[ s_i \, f_i \)$和$\[ s_j \, f_j \)$没有重叠，那么活动$a_i$和$a_j$相互兼容。目标：从所有活动中选出一个活动数量最多的相互兼容活动子集。

*思路:*对给定活动序列按照结束时间从小到大排序,然后逐个贪心选择不冲突的即可.

下面证明活动选择问题具有*最优子结构*。

_Proof._定义$S_(i j)$为$a_i$结束后$a_j$开始前的活动集合，定义$A_(i j)$为$S_(i j)$中规模最大且相互兼容的活动子集。$A_(i j)$中包含活动$a_k$，和两个子问题$A_(i k)$和$A_(k j)$。定义$A_(i k) = A_(i j) ∩ S_(i k)$且$A_(k j) = A_(i j) ∩ S_(k j)$，这样有$A_(i j) = A_(i k) union { a_k } union A_(k j)$，所以有$|A_(i j) |= |A_(i k) |+ |A_(k j) |+ 1$。
...
#line()
  ...
  设$S_J$和$S_I$分别为$J$和$I$的可行调度表.对公共作业$i in I ∩ J$，记在$S_J$中$i$于$\[ t \, t + 1 \]$执行，在$S_I$中于$\[ t' \, t' + 1 \]$执行.当$t < t'$时：将$S_J$中$\[ t' \, t' + 1 \]$的作业与$i$交换.当$t' < t$时：对$S_I$做对称调整.
...
思路:对$⟨ u,v ⟩ in E$,若v是叶子结点,那选u永远比选v好(可以覆盖住除了v的其他叶子).
